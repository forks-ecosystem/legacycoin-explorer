...
const navSnip = `<nav><a href="/" class="brand">⛓ LegacyCoin <small>EXPLORER</small></a><div class="navl"><a href="/home">Home</a><a href="/blocks">All Blocks</a></div><form class="sf" action="/search" method="GET"><input type="text" name="q" placeholder="Height, hash, txid or address…"><button type="submit">→</button></form></nav>`
const footSnip = `<footer><span>LegacyCoin (LBTC)</span> Block Explorer · CPU money for everyone</footer>`

// statsSnip — блок статистики сети (как на главной). Включается в страницы
// home/block/tx/address; использует данные .Info (NodeInfo) и .Mining (MiningInfo),
// заполняемые через s.pageData().
const statsSnip = `{{if .NodeOnline}}<div class="sg">
...
{{if .Error}}<div class="offline">⚠ {{.Error}}</div>{{end}}` + statsSnip + `
...
{{define "block"}}<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Block {{.Block.Height}} — LegacyCoin Explorer</title><style>` + sharedCSS + `</style></head><body>` + navSnip + `<div class="c">` + statsSnip + `
...
<div class="dr"><span class="dk">Block Reward</span><span class="dv gold">{{formatLBTC (blockReward .Block.Height)}}</span></div>
...
{{define "error"}}<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Not Found — LegacyCoin Explorer</title><style>` + sharedCSS + `</style></head><body>` + navSnip + `<div class="c"><div class="ebox"><h2>Not Found</h2><p>{{.Message}}</p><a href="/home">← Home</a></div></div>` + footSnip + `</body></html>{{end}}
...
</script></head><body>` + navSnip + `<div class="c">` + statsSnip + `
